...
Until all five secrets are set, `rate-batch` falls back to the simulated Estes Express estimate (same as every other carrier) rather than failing the batch. The reason for the most recent fallback (auth failure, a rate-quotes error, or a 200 with no usable rate) is persisted to `shipments.estes_last_error` (migration `0008_estes_last_error.sql`, cleared to `NULL` on a successful live quote) and shown directly under the quote table in the UI — no need to have `wrangler tail` running at the exact moment of the request.
...
- Login, shipment intake (paste or sample-email buttons), AI extraction with inline correction, per-shipment rate shopping ("Rate this shipment" on each queued card, "Re-Rate Shipment" once rated), per-shipment booking, and per-shipment quote export.
...
- Live Estes Express rate quotes (FR-3.3): `rate-batch` calls the real Estes Cloud API (`src/estes.ts`) and swaps a live quote in for the simulated Estes Express estimate — pending the five `ESTES_*` secrets above. A live quote shows a blue **LIVE** badge next to the carrier name (migration `0007_carrier_quote_live_flag.sql`, `carrier_quotes.is_live`), plus transit days when Estes returns one — the badge is the reliable signal, not the transit days, since a live quote without transit days would otherwise look identical to a simulated one. Falls back to the simulated estimate (no badge) if the call fails. Every other carrier (XPO, Old Dominion, SAIA, R+L, UPS Freight) is still simulated-only. Booking/tendering with Estes (BOL, pickup requests) is not built.
