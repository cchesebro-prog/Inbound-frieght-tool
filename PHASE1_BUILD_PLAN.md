...
4. ✅ **Booking, export, batch actions** — book/export per shipment and batch-wide (FR-4.x): quote table with best-rate highlight, per-shipment Book/Export, batch Book all/Export all (combined text download). FR-4.5/4.6 (2026-07-28) added a post-booking "carrier confirmation" panel (records the manually-obtained PRO/confirmation number) and a generated Bill of Lading download, available once a shipment is booked.
...
7. ⏳ **PO matching (pulled forward from Phase 3, FR-6.1)** — code complete (`src/acumatica.ts`, PO-related routes, UI lookup/confirm/flag panel), blocked on IT provisioning the three remaining `ACUMATICA_*` secrets (`ACUMATICA_BASE_URL`, `ACUMATICA_CLIENT_ID`, `ACUMATICA_CLIENT_SECRET` — see `README.md`; the fourth, `ACUMATICA_ENDPOINT_VERSION`, is confirmed and no longer a secret, see the 2026-07-28 update in section 3). Acumatica write-back (FR-6.2) remains out of scope until Phase 3.
...
- PO matching (FR-6.1) pulled forward from Phase 3: migration `0003_po_matching.sql` and PO-lookup/confirm/flag routes + UI are built (see milestone 7). `ACUMATICA_ENDPOINT_VERSION` is confirmed (`25.200.001`) and set directly in `wrangler.toml` as of 2026-07-28 — no longer a setup blocker. Still blocked on IT provisioning `ACUMATICA_BASE_URL`, `ACUMATICA_CLIENT_ID`, `ACUMATICA_CLIENT_SECRET` as Worker secrets.
...
- Estes Express live rate quotes (FR-3.3) are code-complete (`src/estes.ts`, wired into `/api/shipments/rate-batch` in `src/index.ts`), blocked on IT/whoever holds the account setting the four `ESTES_*` secrets (`ESTES_API_KEY`, `ESTES_USERNAME`, `ESTES_PASSWORD`, `ESTES_ACCOUNT_NUMBER` — see `README.md` "Estes Express setup"). Until set, `rate-batch` transparently falls back to the simulated Estes Express estimate. Booking (BOL/pickup-requests) remains out of scope — see the 2026-07-28 update in section 8.
